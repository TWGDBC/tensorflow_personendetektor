...
class GridEYEKit():
    def __init__(self):
        ...
        self.tarr_queue = queue.Queue(1)
        self.thermistor_queue = queue.Queue(1)
    ...
    def serial_readline(self,eol=[42,42,42], bytes_timeout=300):
        ...
        line = bytearray()
        while True:
            c = bytearray(bytes(self.ser.readline()))
            if c:
                line = list(c)
                print(line[0:3])
                if line[-0:3] == eol:
